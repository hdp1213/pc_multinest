...
// Constructors --
//----------------
ClassEngine::ClassEngine(const ClassParams& pars): m_cl(0), m_do_free(true), m_pbh_info(0) {
  write_pars_to_fc(pars, &m_fc);

  // Initialise input
  if (input_init(&m_fc, &m_pr, &m_ba, &m_th, &m_pt, &m_tr, &m_pm, &m_sp, &m_nl, &m_le, &m_op, m_pbh_info, m_errmsg) == _FAILURE_) {
...
ClassEngine::ClassEngine(const ClassParams& pars, const std::string & precision_file): m_cl(0), m_do_free(true), m_pbh_info(0) {
...
ClassEngine::ClassEngine(const ClassParams& pars, struct pbh_external* pbh_info): m_cl(0), m_do_free(true), m_pbh_info(pbh_info) {
...
ClassEngine::ClassEngine(const std::string& init_file, int l_max, struct pbh_external* pbh_info): Engine(l_max), m_cl(0), m_do_free(true), m_pbh_info(pbh_info) {
...
double ClassEngine::get_f(double z)
{
  ...
  pvecback = (double *) malloc(m_ba.bg_size*sizeof(double));
