...
CLASS_DIR := $(HOME_DIR)/Documents/git-repos/class
MULTINEST_DIR := $(HOME_DIR)/MultiNest_v3.10
LIB_DIR := $(HOME_DIR)/plc-2.0
...
BATCH_LIB_FLAGS = -llapack -lblas -ldl -lgfortran -lgomp -lclik -lcfitsio -L$(LIB_DIR)/lib -L/usr/lib64/mpich/lib

# Flags for the Fortran compiler which compiles the .o files into the final binary when adding MultiNest
FC := gfortran
FC_FLAGS := -ffree-line-length-none -O4
FC_LIBS := -L$(MULTINEST_DIR) -lnest3 -lstdc++

# Flags for the MPI compilers
FC_MPI := mpifort
FC_MPI_FLAGS := -ffree-line-length-none -O4 -DMPI
